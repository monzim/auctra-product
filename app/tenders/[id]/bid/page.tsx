...
import { ArrowLeft, Upload, AlertCircle, Shield, CheckCircle, Clock, Trash2 } from "lucide-react"
...
export default function SubmitBidPage() {
  ...
  const generateDigitalSignature = async () => {
    ...
    const signature = `DS_${user?.name?.replace(/\s+/g, "").toUpperCase()}_${Date.now()}`
  ...
  const handleLinkProduct = () => {
    ...
    const linkedProduct: LinkedProduct = {
      ...
      quantity,
      totalValue,
      priceVerificationHash: product.priceHistory[product.priceHistory.length - 1]?.blockchainHash || "0x000",
      productBlockchainHash: product.blockchainVerifications[0]?.blockHash || "0x000",
    }
  ...
  const handleRemoveProduct = (productId: string) => {
    const productToRemove = formData.linkedProducts.find((p) => p.productId === productId)
    if (productToRemove) {
      setFormData((prev) => ({
        ...prev,
        linkedProducts: prev.linkedProducts.filter((p) => p.productId !== productId),
        amount: (Number(prev.amount || 0) - productToRemove.totalValue).toString(),
      }))
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    const internalBlockchainHash = `0x${Math.random().toString(16).substr(2, 40)}`
    const publicBlockchainHash = `0x${Math.random().toString(16).substr(2, 32)}`
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Provide your competitive bid details</CardDescription>
                </CardHeader>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Select all applicable certifications you possess</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Link marketplace products to lock in current prices and prevent manipulation
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-muted-foreground mt-1">
                              ${product.lockedPrice.toLocaleString()} × {product.quantity} = $
                              {product.totalValue.toLocaleString()}
                            </div>
                            <div className="text-xs text-muted-foreground">
                              Locked: {new Date(product.lockTimestamp).toLocaleString()}
                            </div>
                          </div>
                  ...
                  <Dialog open={isProductDialogOpen} onOpenChange={setIsProductDialogOpen}>
                    ...
                    <DialogContent className="max-w-2xl">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Select a product from the marketplace to lock in the current price
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">
                              ...
                              return product ? (
                                <div>
                                  ...
                                  <div className="mt-2 flex items-center gap-4">
                                    <Badge variant="outline">Current Price: ${product.price.toLocaleString()}</Badge>
                                    <Badge variant="outline">Vendor: {product.vendorName}</Badge>
                                  </div>
                                </div>
                              ) : null
                            })()}
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <Button onClick={handleLinkProduct} disabled={!selectedProduct || !productQuantity}>
                            Link Product
                          </Button>
                        </div>
                      </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Upload relevant documents to support your bid</CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Generate a secure digital signature for your bid with blockchain storage
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {!formData.digitalSignature ? (
                    <Button onClick={generateDigitalSignature} disabled={isGeneratingSignature} className="w-full">
                      {isGeneratingSignature ? "Generating Signature..." : "Generate Digital Signature"}
                    </Button>
                  ) : (
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 text-green-600">
                        <CheckCircle className="h-4 w-4" />
                        <span className="text-sm font-medium">Digital signature generated</span>
                      </div>
                      <p className="text-xs font-mono bg-muted p-2 rounded break-all">{formData.digitalSignature}</p>

                      <div className="space-y-2 text-xs text-muted-foreground">
                        <div className="flex items-center gap-2">
                          <Shield className="h-3 w-3" />
                          <span>Bid will be stored on internal blockchain with public hash verification</span>
                        </div>
                        <div className="pl-5">
                          <p>• Full bid data: Internal blockchain network</p>
                          <p>• Verification hash: Public blockchain network</p>
                          <p>• Linked product prices: Immutable at submission time</p>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-start space-x-2">
                    ...
                    <label htmlFor="terms" className="text-sm leading-relaxed">
                      I acknowledge that I have read and agree to the tender terms and conditions, and that all
                      information provided in this bid is accurate and complete. I understand that this bid is legally
              ...
              <Button onClick={handleSubmit} className="w-full" size="lg" disabled={isSubmitting}>
                {isSubmitting ? "Submitting Bid to Blockchain..." : "Submit Bid"}
              </Button>
            </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{tender.bidCount} bids submitted</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm">Ensure all required certifications are selected</p>
                  </div>
                  <div className="flex items-start gap-2">
                    <CheckCircle className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                    <p className="text-sm">Provide detailed technical proposal</p>
                  </div>
                  <div className="flex items-start gap-2">
                    <CheckCircle className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    {formData.linkedProducts.map((product) => (
                      <div key={product.productId} className="text-sm">
                        ...
                        <p className="text-muted-foreground">
                          {product.quantity} × ${product.lockedPrice.toLocaleString()} = $
                          {product.totalValue.toLocaleString()}
                        </p>
                      </div>
                    ))}
                    ...
                    <div className="font-medium">
                      Total Linked: $
                      {formData.linkedProducts.reduce((sum, p) => sum + p.totalValue, 0).toLocaleString()}
                    </div>
                  </CardContent>
                </Card>
              )}
