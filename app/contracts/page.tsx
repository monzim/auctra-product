...
const getContractProgress = (milestones: any[]) => {
  const completedMilestones = milestones.filter(
    (m) =>
      m.status === "completed" || m.status === "verified" || m.status === "paid"
  ).length;
            ...
            <div className="text-2xl font-bold">
              {
                mockContracts.filter(
                  (c) => getContractStatus(c.milestones) === "in_progress"
                ).length
              }
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {
                mockContracts.filter(
                  (c) => getContractStatus(c.milestones) === "completed"
                ).length
              }
            </div>
          </CardContent>
...
function VendorContractsView() {
  ...
  const myContracts = mockContracts.filter(
    (contract) => contract.vendorId === user?.id
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Track your active contracts and project milestones
        </p>
      </div>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {myContracts.reduce(
                (sum, contract) =>
                  sum +
                  contract.milestones.filter((m) => m.status === "in_progress")
                    .length,
                0
              )}
            </div>
